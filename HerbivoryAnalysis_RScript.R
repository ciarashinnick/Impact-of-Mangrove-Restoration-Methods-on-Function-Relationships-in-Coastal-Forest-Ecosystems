...
#Impact of vegetation density (CaCo)#############################################
...
# leafarea_z was created as scale(TOTAL_RESTORED_LEAF_AREA_mm)
leaf_mean <- mean(dat_sel$TOTAL_RESTORED_LEAF_AREA_mm, na.rm = TRUE)
leaf_sd   <- sd(dat_sel$TOTAL_RESTORED_LEAF_AREA_mm,   na.rm = TRUE)


#Build prediction grid
# - leafarea_z varies across observed range
# - DBH_z and CaCo_z fixed at 0 (mean values)
# - QUADRAT_TYPE varies (separate lines)
# - PLANT_SPECIES fixed to reference level
# - QUADRAT = NA → population-level predictions

newdat_leaf <- expand.grid(
  leafarea_z = seq(min(dat_sel$leafarea_z, na.rm = TRUE),
                   max(dat_sel$leafarea_z, na.rm = TRUE),
                   length.out = 100),
  DBH_z        = 0,
  CaCo_z       = 0,
  QUADRAT_TYPE = levels(dat_sel$QUADRAT_TYPE),
...
#Attach predictions and back-transform leaf area

newdat_leaf <- newdat_leaf %>%
  mutate(
    fit = pred_leaf$fit,
    se  = pred_leaf$se.fit,
    lwr = fit - 1.96 * se,
    upr = fit + 1.96 * se,
    leafarea_mm = (leafarea_z * leaf_sd + leaf_mean)
  )

#Plot leaf area effect

p_leaf <- ggplot(newdat_leaf,
                 aes(x = leafarea_mm,
                     y = fit,
                     colour = QUADRAT_TYPE,
                     fill = QUADRAT_TYPE)) +
...
ggsave("Fig_leafarea_predicted_herbivory.png",
       p_leaf, width = 6.5, height = 4.5, dpi = 600)

ggsave("Fig_leafarea_predicted_herbivory.pdf",
       p_leaf, width = 6.5, height = 4.5)

#Leaf area summary statistics by forest type
leaf_by_type <- dat_sel %>%
  group_by(QUADRAT_TYPE) %>%
  summarise(
    mean_leaf = mean(TOTAL_RESTORED_LEAF_AREA_mm, na.rm = TRUE),
    sd_leaf   = sd(TOTAL_RESTORED_LEAF_AREA_mm, na.rm = TRUE),
    n = n(),
    .groups = "drop"
  )
...
# Compute mean/sd for back-transforming leafarea_z ticks
leaf_mean <- mean(dat_sel$TOTAL_RESTORED_LEAF_AREA_mm, na.rm = TRUE)
leaf_sd   <- sd(dat_sel$TOTAL_RESTORED_LEAF_AREA_mm,   na.rm = TRUE)


z_breaks_leaf <- pretty(range(dat_sel$leafarea_z, na.rm = TRUE), n = 5)
# Convert to original units (mm²)
leaf_labels <- round(z_breaks_leaf * leaf_sd + leaf_mean, 1)

# sjPlot prediction plot for leaf area
p_leaf_sj <- plot_model(
  m_final3,
  type  = "pred",
  terms = c("leafarea_z [all]", "QUADRAT_TYPE")
) +
  labs(x = "Leaf area (mm²)",
       y = "Predicted herbivory proportion",
       colour = "Quadrat type",
       fill = "Quadrat type",
       title = "Likupang") +   
  scale_x_continuous(breaks = z_breaks_leaf, labels = leaf_labels) +
  pub_theme() +
  theme(
    plot.title = element_text(hjust = 0.5, family = "Times New Roman", face = "bold"),
    text = element_text(family = "Times New Roman")
...
ggsave("Fig_effect_leafarea_quadratType.png",
       p_leaf_sj, width = 7, height = 5, dpi = 600)

ggsave("Fig_effect_leafarea_quadratType.pdf",
       p_leaf_sj, width = 7, height = 5)
